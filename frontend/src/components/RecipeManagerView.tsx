...
export const RecipeManagerView: React.FC<Props> = ({
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fadeIn pb-16">
      ...
      <div className="bg-white rounded-3xl p-3.5 sm:p-4 border-2 border-slate-200/90 shadow-sm space-y-3">
        {/* Search Bar */}
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-9 py-2 rounded-xl border-2 border-slate-200 text-xs sm:text-sm font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 placeholder:text-slate-400 bg-slate-50"
          />
          {searchTerm && (
            <button
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-1">
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-black transition shrink-0 shadow-2xs ${
                quickFilter === f.id
      ...
      {/* Results Count Strip */}
      <div className="flex items-center justify-between px-2 text-xs text-slate-600 font-bold">
        <span>
      ...
      {isLoading ? (
        <div className="p-12 text-center text-slate-400 bg-white rounded-3xl border-2 border-slate-200">
          <ChefHat className="w-8 h-8 mx-auto mb-2 animate-bounce text-emerald-600" />
          <p className="text-xs font-bold">Lade Rezepte...</p>
        </div>
      ) : filteredRecipes.length === 0 ? (
        <div className="p-10 text-center bg-white rounded-3xl border-2 border-slate-200 max-w-md mx-auto space-y-3">
          ...
          {filteredRecipes.map((recipe) => {
            ...
            return (
              <div
                ...
                className="bg-white rounded-2xl border-2 border-slate-200/90 hover:border-emerald-500 shadow-sm hover:shadow-md transition duration-150 cursor-pointer overflow-hidden flex flex-col justify-between group"
              >
                <div className="p-3.5 sm:p-4">
                  {/* Top Row: Thumbnail + Details */}
                  <div className="flex items-start gap-3">
                    {/* Compact 80x80 Photo Thumbnail */}
                    <div className="w-20 h-20 rounded-2xl bg-slate-900 overflow-hidden shrink-0 relative shadow-xs aspect-square ring-1 ring-slate-300">
                      <img
                    ...
                    <div className="min-w-0 flex-1 space-y-1.5">
                      <div className="flex items-center justify-between gap-1">
                        ...
                        <div className="flex items-center gap-1 opacity-80 group-hover:opacity-100">
                          <button
                            ...
                            className="p-1 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-100 border border-slate-200/60 transition"
                            title="Bearbeiten"
                          >
                          ...
                          <button
                            ...
                            className="p-1 rounded-lg text-slate-500 hover:text-red-600 hover:bg-red-50 border border-slate-200/60 transition"
                            title="Löschen"
                          >
                      ...
                      <h3 className="text-xs sm:text-sm font-black text-slate-900 leading-snug line-clamp-2 group-hover:text-emerald-700 transition">
                        {recipe.title}
                      </h3>

                      {/* Key Macros Strip */}
                      <div className="flex items-center gap-1.5 text-[11px] font-bold flex-wrap">
                        <span className="flex items-center gap-0.5 bg-slate-100 border border-slate-200 text-slate-800 px-1.5 py-0.5 rounded-md">
                          <Clock className="w-3 h-3 text-slate-500" />
                          {totalTime}m
                        </span>
                        <span className="flex items-center gap-0.5 bg-orange-50 border border-orange-200 text-orange-950 px-1.5 py-0.5 rounded-md">
                          <Flame className="w-3 h-3 text-orange-600 fill-orange-500" />
                          {recipe.base_calories}
                        </span>
                        <span className="bg-blue-50 border border-blue-200 text-blue-950 px-1.5 py-0.5 rounded-md">
                ...
                {/* Card Action Strip */}
                <div className="px-3.5 sm:px-4 py-2 bg-slate-50/90 border-t-2 border-slate-100 flex items-center justify-between gap-2 text-xs">
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl bg-white hover:bg-slate-100 text-slate-800 font-bold text-[11px] border border-slate-200 shadow-2xs transition">
